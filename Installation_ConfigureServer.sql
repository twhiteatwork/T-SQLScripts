...
--Enable Database Mail extended procedures
...
EXEC msdb.dbo.sp_add_operator @name=N'DBA', 
		...
		@pager_days=0, 
		@email_address=N'channel.dba@vistex.com', 
		@category_name=N'[Uncategorized]'
GO

--Enable Mail Profile and Setup the failsafe alert operator in SQL Agent properties

--Create alerts for major failure error instances
...
GO
EXEC master.dbo.xp_instance_regwrite N'HKEY_LOCAL_MACHINE', N'SOFTWARE\Microsoft\MSSQLServer\SQLServerAgent', N'DatabaseMailProfile', N'REG_SZ', N'channel.dba@vistex.com'
GO
...
GO
EXEC msdb.dbo.sp_add_notification @alert_name=N'Severity 017', @operator_name=N'Helpdesk', @notification_method = 1
GO
EXEC msdb.dbo.sp_add_notification @alert_name=N'Severity 018', @operator_name=N'Helpdesk', @notification_method = 1
GO
EXEC msdb.dbo.sp_add_notification @alert_name=N'Severity 019', @operator_name=N'Helpdesk', @notification_method = 1
GO
EXEC msdb.dbo.sp_add_notification @alert_name=N'Severity 020', @operator_name=N'Helpdesk', @notification_method = 1
GO
EXEC msdb.dbo.sp_add_notification @alert_name=N'Severity 021', @operator_name=N'Helpdesk', @notification_method = 1
GO
EXEC msdb.dbo.sp_add_notification @alert_name=N'Severity 022', @operator_name=N'Helpdesk', @notification_method = 1
GO
EXEC msdb.dbo.sp_add_notification @alert_name=N'Severity 023', @operator_name=N'Helpdesk', @notification_method = 1
GO
EXEC msdb.dbo.sp_add_notification @alert_name=N'Severity 024', @operator_name=N'Helpdesk', @notification_method = 1
GO
EXEC msdb.dbo.sp_add_notification @alert_name=N'Severity 025', @operator_name=N'Helpdesk', @notification_method = 1
GO
EXEC msdb.dbo.sp_add_notification @alert_name=N'Error Number 823', @operator_name=N'Helpdesk', @notification_method = 1
GO
EXEC msdb.dbo.sp_add_notification @alert_name=N'Error Number 824', @operator_name=N'Helpdesk', @notification_method = 1
GO
EXEC msdb.dbo.sp_add_notification @alert_name=N'Error Number 825', @operator_name=N'Helpdesk', @notification_method = 1
GO
...
SET @BackupDirectory     = N'F:\Backup\Default' -- Specify the backup root directory.
...
--		sum(
--			(convert(bigint, datediff(minute, login_time, getdate())) * 60000)
--			+ datediff(millisecond, dateadd(minute, datediff(minute, login_time, getdate())	, getdate()), getdate())
--)
--		- 
...
--Move and resize tempdb files if default instance, else will need to be run separately with scripts adjusted for instance name
use master
go
if @@servicename = 'MSSQLSERVER'
begin
	alter database tempdb MODIFY FILE (name='tempdev', filename='d:\default\MDF_Files\tempdev.mdf', size = 2GB, filegrowth = 0)
	alter database tempdb MODIFY FILE (name='templog', filename='d:\default\LDF_Files\templog.ldf' , size = 2GB, filegrowth = 0)
	ALTER DATABASE [tempdb] ADD FILE ( NAME = N'tempdev_ndf1', filename='d:\default\NDF_Files\tempdev_ndf1.ndf' , SIZE = 2GB , FILEGROWTH = 0)
	ALTER DATABASE [tempdb] ADD FILE ( NAME = N'tempdev_ndf2', filename='d:\default\NDF_Files\tempdev_ndf2.ndf' , SIZE = 2GB , FILEGROWTH = 0)
	ALTER DATABASE [tempdb] ADD FILE ( NAME = N'tempdev_ndf3', filename='d:\default\NDF_Files\tempdev_ndf3.ndf' , SIZE = 2GB , FILEGROWTH = 0)
end
...
--Increase max number of job history rows
USE [msdb]
GO
...
--Set default file growth in model database
USE [master]
GO
